...
class Syllogism:
    ...
    def __init__(self, premise1: str, premise2: str, formalized=True):
        ...
        self.terms_dict = dict(
            Counter(t for terms in premises_in_init for t in terms if t == t.upper()))
        ...
        self.midterm = "".join(
            [k for k, v in self.terms_dict.items() if v == 2])
    ...
    def directive_one(self) -> bool:
        ...
        :rtype: bool
        '''
        substrings = [f"{self.midterm}a", f"{self.midterm}e",
                      f"e{self.midterm}", f"o{self.midterm}"]
